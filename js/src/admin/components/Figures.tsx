...
export default class Figures extends Component<FiguresAttrs> {
  view(): Mithril.Children {
    return (
      <div className="PlacementFigures">
        {this.attrs.figures.map((figure) => (
          <div className="PlacementFigure">
            <div className="PlacementFigure-value">{figure.value}</div>
            <div className="PlacementFigure-label">
              {figure.help ? (
                <Tooltip text={extractText(figure.help)}>
 ...
 * to a sponsor.
 */
export const RATE_THRESHOLD = 1000;
 ...
 */
export function rate(numerator: number, denominator: number, threshold: number = RATE_THRESHOLD): Mithril.Children {
  if (denominator <= 0 || denominator < threshold) return '—';
